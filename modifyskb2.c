...
//http://www.programmershare.com/943964/
...
int (*okfn)(struct sk_buff *))
{
    ...
    if(iph->daddr == in_aton("173.26.100.224"))
    {
        ...
        skb_pull(skb, iph_len);//skb->data
        skb_reset_transport_header(skb);/*,ip*/
        if(iph->protocol == IPPROTO_TCP)
        {
            ...
            if(tcph->dest == htons(3306)) //
            {
                ...
                skb->dev = dev;
                skb_push(skb, iph_len);/*?skbL3*/
                //skb_reset_transport_header(skb);
                skb_push(skb, ETHALEN);//skb->datal2?dev_queue_xmit()
